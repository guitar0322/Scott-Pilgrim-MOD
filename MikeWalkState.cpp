...
EnemyState* MikeWalkState::Update(EnemyAI* enemy)
{
...
void MikeWalkState::Enter(EnemyAI* enemy)
{
	_moveStartTime = 0.0f;
	_moveMaxTime = 5.0f;
	...
	if (enemy->enemyinfo->GetDir() == false)
		enemy->ChangeClip("mike_walk_right", true);
	else
		enemy->ChangeClip("mike_walk_left", true);
}

void MikeWalkState::Exit(EnemyAI* enemy)
{
}
